...
import utils as UT
from model.ALICC import ALICC as Network

# ======================================================================================================================
weights_dir = '/home/tdx/ypw/ALICE/checkpoints/Real/model_best.pth'  #LDRM
# weights_dir = '/home/tdx/桌面/Project/databak/EXT4/YPW/ALICC/checkpoints_ALICC/ALICC_dwtloss2/Real/Pretrain/model_best.pth' #LDRMv2
data_dir = '/home/tdx/ypw/simulate_data/uavid_v1.5_official_release_image/Real8.31/Test/Test'
# data_dir = '/home/tdx/桌面/Project/databak/EXT4/YPW/ALICC/LCM/Real_dinxing_1'
result_dir = './results/LDRM/Real_dinxing'
...
class TEST:
    ...
    def single_img(self, data_dir, result_dir):
        ...
        with torch.no_grad():
            ...
            UT.save_img((os.path.join(result_dir, img_name + '.png')), img_as_ubyte(restored_img))
        ...
        with torch.no_grad():
            ...
            for ii, data in pbar:
                ...
                for batch in range(len(restored)):
                    UT.save_img((os.path.join(result_dir, f'{filenames[batch]}.png')), img_as_ubyte(restored[batch]))
    ...
    def refer(self, data_dir, result_dir):
        val_dataset = UT.DataLoaderVal(data_dir, patch_size=None, Scale=1, ColorSet='Color/')
        ...
        with torch.no_grad():
            ...
            for ii, data in pbar:
                ...
                start_time = time.time()
                # Assuming the model now returns a tuple with restored, dec, and h_map
                restored,_,_,_ = self.model(color, mono)
                ...
                ssim = ssim_loss(restored, gt, win_size=3, multichannel=True, data_range=1.0)
                psnr_val_rgb.append(psnr)
                ssim_val_rgb.append(ssim)

                UT.save_img(os.path.join(result_dir, filenames[0] + '.png'), img_as_ubyte(restored))

                info = f'One Batch Time:{pred_time:.4f} Image:{H, W} PSNR:{psnr:.4f} SSIM:{ssim:.4f} Total Processing Time:{time_count:.4f}'
                pbar.set_description(info)

            psnr_val_rgb = sum(psnr_val_rgb) / len(val_loader)
            ssim_val_rgb = sum(ssim_val_rgb) / len(val_loader)
            print(f"Avg One Img Time: {time_count / len(val_loader)} \nPSNR: {psnr_val_rgb}  SSIM: {ssim_val_rgb}")
